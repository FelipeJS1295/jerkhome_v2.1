...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="overflow-x-auto bg-white rounded-lg shadow">
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr class="{{ isset($venta['ya_existe']) && $venta['ya_existe'] ? 'bg-red-100' : 'hover:bg-gray-50' }}">
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $venta['numero_orden'] ?? 'N/A' }}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $venta['fecha_entrega'] ?? 'N/A' }}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $venta['sku'] ?? 'N/A' }}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $venta['producto'] ?? 'N/A' }}</td>
    ...
    <div class="mt-6">
        <form action="{{ route('ventas.guardarPreviewLapolar') }}" method="POST">
            @csrf
